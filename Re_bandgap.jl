...
function ReΔ(α²F::F, u, ωc, ω₀, λ₀, λ₁, ϵ, ωcrit, T, maxiter, N, maxω, kₘ, Δₘ, kB, zmin, zmax) where {F<:Function}
    ...
    ω = LinRange(-maxω, maxω, 2N);      dω = abs(ω[1]-ω[2]);                M = size(kₘ,1)
    ...
    α²Fᵢ = α²F(ω.+maxω, ω₀, λ₀, λ₁, ϵ, ωcrit, zmin, zmax)
    ...
    for l ∈ 1:maxiter
        ...
        @inline for n ∈ -N:N-1  #ω
            ...
            SR .= sqrt.(Complex.((ω .* Z).^2 .- W.^2))
            nᵢ = n+N+1;     Lᵢ = N-n

            @views n1[1:Lᵢ] .= n_B.(ω[1:Lᵢ].+ maxω,T,δ,kB) .+ n_F.(ω[nᵢ:end],T,δ,kB)
            @views n2[1:nᵢ] .= n_B.(ω[1:nᵢ].+ maxω,T,δ,kB) .+ n_F.(-ω[nᵢ:-1:1],T,δ,kB)
            
            #∫dz 
            @views sumW1[1:Lᵢ] .= α²Fᵢ[1:Lᵢ] .* ( n1[1:Lᵢ] .* (W[nᵢ:end] ./ SR[nᵢ:end]) ) .* sign.(imag.(SR[nᵢ:end]) )
            @views sumZ1[1:Lᵢ] .= α²Fᵢ[1:Lᵢ] .* ( n1[1:Lᵢ] .* (ω[nᵢ:end].*Z[nᵢ:end] ./ SR[nᵢ:end]) ) .* sign.(imag.(SR[nᵢ:end]) )
            #∫(-dz) 
            @views sumW2[1:nᵢ] .= α²Fᵢ[1:nᵢ] .* ( n2[1:nᵢ] .* (W[nᵢ:-1:1] ./ SR[nᵢ:-1:1]) ) .* sign.(imag.(SR[nᵢ:-1:1]) )
            @views sumZ2[1:nᵢ] .= α²Fᵢ[1:nᵢ] .* ( n2[1:nᵢ] .* (ω[nᵢ:-1:1].*Z[nᵢ:-1:1] ./ SR[nᵢ:-1:1]) ) .* sign.(imag.(SR[nᵢ:-1:1]) )
            ...
            Z[nᵢ] = 1 + Den[nᵢ] + (im*π*dω/ω[nᵢ]) * (IntZ1 + IntZ2)
            W[nᵢ] = Num[nᵢ] + im*π*dω * (IntW1 + IntW2)
